(function(app){
    if (app) {
        ...
        app.Product = function(response) {
            ...
            model.colorVariationVals = (function(){
                var atts = model.variations.attributes;             
                for (var i=0,il=atts.length;i<il;i++) {                 
                    if (atts[i].id==="color") {     
                        return atts[i].vals;
                    }               
                }
            ...
            var loadProductImageSet = function(product) {
                loadImageSet(model.images, product, function() {
                    jQuery(product).trigger("ProductImageSetLoaded");
                });
            };          
            
            var loadVariantImageSet = function(product, index) {
                if (!model.colorVariationVals || !model.colorVariationVals[index]) {
                    jQuery(product).trigger("VariationImageSetsLoadComplete");
                    return;
                }
                loadImageSet(model.colorVariationVals[index].images, product, function() {
                    index++;
                    if (index >= model.colorVariationVals.length) {
                        jQuery(product).trigger("VariationImageSetsLoadComplete");
                    } 
                    else {
                        jQuery(product).trigger("VariationImageSetLoaded", [index]);
                    }
                });
            };          
            
            var loadImageSet = function(imgs, product, callback) {
                $.ajax({
                     // url: imgs.imageSetUrl+",json",
                      dataType: 'jsonp',                      
                      jsonpCallback: 's7jsonResponse',
                      success: function(data) {
                        if (!data || !data.IMAGE_SET || data.IMAGE_SET.length==0) {
                            return;
                        }                       
                        var segs = data.IMAGE_SET.split(";");
                        for (var i=0,il=segs.length;i<il;i++) {                         
                            var imgPath = segs[i].split(",")[0];
                            var imgUrl = app.URLs.s7BaseUrl+imgPath;                            
                            if ($.inArray(imgUrl+"?$large$", imgs.large) === -1) {
                                imgs.large.push(imgUrl+"?$large$");
                            }
                            if ($.inArray(imgUrl+"?$productalt$", imgs.small) === -1) {
                                imgs.small.push(imgUrl+"?$productalt$");
                            }
                        }
            ...
            var getAddToCartBtn = function(thisProduct) {
                var addToCartBtn = jQuery(thisProduct.containerId+" .addtocartbutton:last").click(function(e) {
                    ...
                    thisProduct.selectedOptions.cgid = thisProduct.cgid;
                    thisProduct.selectedOptions.start = thisProduct.start;
                    thisProduct.selectedOptions.srule = thisProduct.srule;
            ...
            var getMiscLinks    = function(thisProduct) {
                ...
                jQuery(thisProduct.containerId+" .sendtofriend").click(function(e) {                    
                    ...
                    jQuery("body").removeClass("quickview-active");// added to fix scene7 zoom player overlapping STAF layer
            ...
            var createAvMessage = function(thisProduct, val) {
                ...
                // display custom message if less than 5 units left.
                if (ats < 5 && ats > 0 && inStockLevel > 0) {
                    avMessage = "<span class='invmessage'>Only " + parseInt(ats) + " left in this combination.</span>";
                }
                
                // display custom message for not available
                if (ats == 0) {
                    avMessage = "<span class='notavailable'>Not in Stock</span>";
                }
            ...
            return {
                ...
                showImages: function(selectedVal, vals)  {
                    ...
                    var that = this,
                        $itemContainer = jQuery(that.containerId);
                        $imageContainer = $itemContainer.find(".productimage"),
                        $thumbsContainer = $itemContainer.find(".productthumbnails:last");
                        
                    vals = vals || {};
                    function zoomPrimaryImage(primaryImageSrc, hiResImageSrc){
                        
                        if(jQuery('body').hasClass('quickview-active')){
                            var zoomParams = "transition:none,1;flyout:310,400,300,-1;zoomFactor:2.5;frameBorder:#e0e0e0 1px dashed;flyoutBorder:#e0e0e0 1px solid;dimColor:none;z-index:85;"
                            jQuery(".primaryproductimage img").attr("id","flyzoom").attr("params",zoomParams);
                            sjFlyout("flyzoom",primaryImageSrc, hiResImageSrc);
                        } 
                        else {
                            var zoomParams = "transition:none,1;flyout:400,600,450,-1;zoomFactor:2.5;frameBorder:#e0e0e0 1px dashed;flyoutBorder:#e0e0e0 1px solid;dimColor:none;z-index:85;"
                            jQuery(".primaryproductimage img").attr("id","flyzoom").attr("params",zoomParams);
                            sjFlyout("flyzoom",primaryImageSrc, hiResImageSrc);
                        }
                    }   
                    ...
                    jQuery.each(vals, function(){
                        ...
                        if (this.val === selectedVal && this.images) {
                            ...
                            if (this.images.small.length > 0) {                             
                                var pimg = $imageContainer.find("img");                             
                                var vimgs = thisVal.images;
                                var map = {src:vimgs.large[0], alt:vimgs.alt, title:vimgs.title};
                                if (pimg.length===0) {                                  
                                    pimg = $("<img/>").attr(map).appendTo($imageContainer);
                                } 
                                else if (pimg.src!=vimgs.large[0]){                                 
                                    pimg.attr(map);
                                }                               
                    
                                var productsetpage=$('#pdpMain').hasClass('productdetail productdetailset');
                                        // scene seven zoom integration
                                if((!productsetpage || !that.isSubProduct()) && !that.isBonusProduct()){
                                    zoomPrimaryImage(vimgs.large[0], vimgs.hires[0]);
                                }
                                // -------------------------------
                            }

                            var theImgs = this.images;

                            if (that.isSubProduct() && jQuery(app.ProductCache.subProducts).length > 0 && this.images.bundle) {                             
                                $thumbsContainer.append(jQuery("<img/>").attr({src:this.images.bundle, alt:theImgs.alt, title:theImgs.title}));
                            }
                            else if (this.images.small.length > 1 && (jQuery(app.ProductCache.subProducts).length === 0 || !that.isSubProduct()) && !that.isBonusProduct()) {                               
                                
                                jQuery.each(this.images.small, function(idx){                                   
                                    $thumbsContainer.append(jQuery("<img/>").attr({src:this, alt:theImgs.alt, title:theImgs.title}).click(function(e){
                                        var pimg = $imageContainer.find("img");
                                        pimg.attr("src", thisVal.images.large[idx]);                
                                        
                                        // scene seven zoom integration
                                        zoomPrimaryImage(thisVal.images.large[idx], thisVal.images.hires[idx]);
                                        // -------------------------------
                                    }));
                                });                                                             
                            }
                ...
                */
                varAttrSelected: function(e) {
                    ...
                    if (!isLoadingVar) {
                        ...
                        jQuery.each(model.variations.attributes, function (idx) {                           
                            ...
                            else if (e.data.id.toLowerCase()==='color'||e.data.val.toLowerCase()==='color') {                               
                ...
                * */
                ...
                refreshView: function() {
                    ...
                    if (!isLoadingVar && this.selectedVar != null) {
                        ...
                        var available = this.selectedVar.avStatus != app.constants.AVAIL_STATUS_NOT_AVAILABLE && this.selectedVar.inStock;
                        var validPrice = (this.getPrice() > 0) || this.isPromoPrice();
                        if (available && validPrice) {
                            // enable add to cart button
                ...
                showUpdatedPrice: function(sale, standard) {
                    ...
                    if (standardPrice > 0 && standardPrice > salePrice) {
                        // show both prices
                        priceHtml = priceHtml + '<div class="standardprice">' + formattedPrices.standardPrice + ' </div>';                      
                    }                   
                    
                    jQuery(this.containerId+" .productinfo .price:first").html(priceHtml);
                    // update productsets and bundles with second instance of price
                    if(this.containerId === "#pdpMain"){
                        jQuery(this.containerId+" .grouptotal .price:first").html(priceHtml);
                    }
                ...
                */
                ...
                show: function(options) {
                    ...
                    jQuery(this).bind("ProductImageSetLoaded", function(e){
                        loadVariantImageSet(thisProduct, 0);
                    });
                    
                    jQuery(this).bind("VariationImageSetLoaded", function(e, index){                                                
                        loadVariantImageSet(thisProduct, index);
                    });
                    
                    jQuery(this).bind("VariationImageSetsLoadComplete", function(e){
                        // load variants, etc
                        this.showProductImages();
                    });
                    
                    loadProductImageSet(thisProduct);
                    
                    this.containerId    = "#"+options.containerId;                  
                    var isQuickView     = (options.source && options.source == "quickview");
                    ...
                    // render variants and alternate images after alternates are loaded...
                    renderVariants : function(options) {
                        ...
                        jQuery(thisProduct.containerId + " .variationattributes .swatches").each(function(){
                            ...
                            var varEventHandler = function(e){                                  
                                ...
                                //committed out to resolve deselection issue
...
// placed as "stub" for jsonp call
var s7jsonResponse = function(data){};